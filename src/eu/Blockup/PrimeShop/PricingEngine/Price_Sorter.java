...
class Price_Comparator implements Comparator<Shop_Item> {
	...
	public int compare(Shop_Item s1, Shop_Item s2) {
		...
		double price1 =  s1.timesItemWasBought.getValue() - s1.timesItemWasSold.getValue();
		double price2 =  s2.timesItemWasBought.getValue() - s2.timesItemWasSold.getValue();
	...
	public static List<String> get_Sortet_List (int maxLines, boolean ascending) {
		...
		for (int i = 0; i < Item_List.size(); i++) {
			...
			if (i < maxLines) {	
				...
				String itemname = item.itemDisplayname.getValue() + " (" + item.mcItemid + ")";
...
//				String itemPrice = PrimeShop.economy.format(item.calculate_price(item.timesItemWasBought.getValue(), item.timesItemWasSold.getValue()));
//				String itemPrice = PrimeShop.economy.format(item.clone().buyItem(true, 1, false).price);
				resultList.add(Message_Handler.resolve_to_message(132, String.valueOf(i+1), itemname.toUpperCase(), itemPrice));
