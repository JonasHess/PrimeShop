			...
			if (itemA_has_Meta) {
				...
				if ((itemMetaA instanceof PotionMeta) == (itemMetaB instanceof PotionMeta)) {
					if (itemMetaA instanceof PotionMeta) {
						...
						if (!compareEnchantmeList(potionMetaA.getEnchants(),
							...
							return false;
						// TODO compare custom effects potionMetaA.getCustomEffects()
					}
				}
				//LeatherArmorMeta
				if ((itemMetaA instanceof LeatherArmorMeta) == (itemMetaB instanceof LeatherArmorMeta)) {
					if (itemMetaA instanceof LeatherArmorMeta) {
						...
						if (!leatherMetaA.getDisplayName().matches(leatherMetaB.getDisplayName())) return false;
						if (leatherMetaA.getColor() != leatherMetaB.getColor()) return false;
				...
				if ((itemMetaA instanceof FireworkMeta) == (itemMetaB instanceof FireworkMeta)) {
					if (itemMetaA instanceof FireworkMeta) {
						...
						if (!compareEnchantmeList(fireworkMetaA.getEnchants(),
							...
							return false;
						List<FireworkEffect> fireworkEffectListA = fireworkMetaA.getEffects();
						List<FireworkEffect> fireworkEffectListB = fireworkMetaB.getEffects();
						for ( FireworkEffect effect : fireworkEffectListB) {
							if (!(fireworkEffectListB.contains(effect))) return false;
						}
				...
				if ((itemMetaA instanceof BookMeta) == (itemMetaB instanceof BookMeta)) {
					if (itemMetaA instanceof BookMeta) {
						...
						if (!bookMetaA.getTitle().matches(bookMetaB.getTitle())) return false;
						if (!bookMetaA.getAuthor().matches(bookMetaB.getAuthor())) return false;
						// TODO compare pages
