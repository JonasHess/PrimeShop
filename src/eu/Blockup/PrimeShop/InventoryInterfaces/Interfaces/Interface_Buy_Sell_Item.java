...
public class Interface_Buy_Sell_Item extends InventoryInterface {
	...
	public Interface_Buy_Sell_Item(final List<InventoryInterface> link_Back_Stack, Player player,
			...
			final int amount, boolean deleteable, final boolean item_is_bought_without_shop) {
...
//				player.sendMessage("Du hast gedrückt");
		...
		Button_Buy_Sell_Item sell_Button = new Button_Buy_Sell_Item(permissionToSELL, false, this.amount, itemStack_to_be_bought,"");
		...
		if (this.shop != null) {	
			...
			if (PrimeShop.get_all_SubItems_of_ItemStack(
					itemStack_to_be_bought).isEmpty() || PrimeShop.plugin.is_PriceLinking_disabled_for_Item(itemStack_to_be_bought)) {
	
				// Change Price
	
				ItemStack currentItem = new ItemStack(Material.getMaterial(266));
				this.addOption(8, 2,
						new Button(currentItem.getType(), currentItem.getData()
								.getData(), Message_Handler.resolve_to_message(22),
							...
							public void onClick(InventoryInterface inventoryInterface, Player player,
								...
								PrimeShop.databaseHandler.link_with_Databse_if_not_allready_linked(PrimeShop.get_Shop_Item_of_Itemstack(itemStack_to_be_bought));
								PrimeShop.open_InventoyInterface(player,
										new Interface_Change_Price(
												inventoryInterface.branch_back_Stack, player,
				...
				this.addOption(8, 1,
						new Button(currentItem.getType(), currentItem.getData()
								.getData(), Message_Handler.resolve_to_message(24),
				...
				this.addOption(8, 2, new Button(Material.getMaterial(14),
						(short) 1, Message_Handler.resolve_to_message(49), Message_Handler.resolve_to_message(50), Message_Handler.resolve_to_message(51)) {
					...
					public void onClick(InventoryInterface inventoryInterface, Player player, ItemStack cursor,
						...
						PrimeShop.close_InventoyInterface(player);
						PrimeShop.open_InventoyInterface(player,
								new Interface_List_of_Subitems(inventoryInterface.branch_back_Stack,
										itemStack_to_be_bought, shop, player));
					}
				});
			}
			// Enable PriceLinking
					...
					if (PrimeShop.plugin
						...
						addOption(7, 3, new Button(Material.ANVIL,
								Message_Handler.resolve_to_message(52), Message_Handler.resolve_to_message(53)) {
							...
							public void onClick(InventoryInterface inventoryInterface, Player player, ItemStack cursor,
								...
								if (PrimeShop.plugin.is_PriceLinking_disabled_for_Item(itemStack_to_be_bought)) {
									...
									PrimeShop.close_InventoyInterface(player);
									PrimeShop.open_InventoyInterface(player, inventoryInterface.branch_back_Stack
			...
			if (shop_ != null) {
				if (PrimeShop.has_player_Permission_for_this_Command(player, "PrimeShop.admin.addItemsToShop." + shop_.shopname)) {	
				
				// Remove Item from Shop
		
				if (deleteable) {
					this.addOption(8, 3, new Button(Material.WOOL, (short) 14,
							Message_Handler.resolve_to_message(54), Message_Handler.resolve_to_message(55), "") {
						...
						public void onClick(InventoryInterface inventoryInterface, Player player, ItemStack cursor,
							...
							PrimeShop.close_InventoyInterface(player);
							PrimeShop.open_InventoyInterface(player, new Interface_delete_Item_from_Shop(
									inventoryInterface.branch_back_Stack, player,
									itemStack_to_be_bought, shop));
						}
					});
				}
				}
		...
		Button_Amount button_Amount = new Button_Amount(itemStack_to_be_bought, "", 
				Message_Handler.resolve_to_message(56), Message_Handler.resolve_to_message(57), Message_Handler.resolve_to_message(58), Message_Handler.resolve_to_message(59));
		button_Amount.setAmount(this.amount);
		this.addOption(4, 0, button_Amount);
		} else {
			this.addOption(4, 0, new Button_with_no_task(itemStack_to_be_bought, "", Message_Handler.resolve_to_message(56, PrimeShop.convert_IemStack_to_DisplayName(itemStack_to_be_bought)), Message_Handler.resolve_to_message(60)));
		}
		...
		if (!item_is_bought_without_shop) {
			if (position_in_Stack > 0) {
				...
				this.addOption(0, 0, new Button(shop.displayIcon,
						Message_Handler.resolve_to_message(61), Message_Handler.resolve_to_message(62)) {
