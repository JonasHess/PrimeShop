	...
	public Interface_delete_Shop_from_Collection(final List<InventoryInterface> link_Back_Stack, Player player, final Shop shop, final int collectionID, final boolean npc) {
		...
		for (int a = 0; a < this.getWidth(); a++) {
			for (int b = 0; b < this.getHeight(); b++) {
				this.addOption(a, b,
						new Button_with_no_task(Cofiguration_Handler.background_ItemStack, " "));
			}
		}

		// Icon

		this.addOption(4, 0, new Button_with_no_task(shop.displayIcon,
				Message_Handler.resolve_to_message(72), Message_Handler.resolve_to_message(45)));

		// Yes

		
		this.addOption(3, 2, new Button(Material.WOOL, (short) 5, Message_Handler.resolve_to_message(73)) {
			...
			public void onClick(InventoryInterface inventoryInterface, Player player,
				...
				if (npc) {
					if (PrimeShop.plugin.remove_Shop_from_NPC(shop, collectionID)) {
						player.sendMessage(Message_Handler.resolve_to_message(71));
