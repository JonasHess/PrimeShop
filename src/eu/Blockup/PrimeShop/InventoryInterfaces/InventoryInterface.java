...
public class InventoryInterface implements Cloneable {
	...
	// private List<Menu> preapre_link_List_for_Copying () {
	// List<Menu> result = new ArrayList<Menu>(this.branch_back_Stack.size()
	// -1);
	//
	//
	// for (Menu m : this.branch_back_Stack) {
	// result.add(m);
	// }
	//
	...
	// // for (int i = 0; i<this.branch_back_Stack.size(); i++) { // todo -1?
	// // result.add(this.branch_back_Stack.get(i));
	// // }
	// return result;
	// }
	...
	public InventoryInterface setTitle(String title) {
		if (title.length() > 32) {
...
//		for (int i = 0; i < this.branch_back_Stack.size() - 2; i++) {
//			Menu m = this.branch_back_Stack.get(i);
//			list.add(m.clone());
//		}
//
//		Menu menu = new Menu(title, this.getHeight(), list);
